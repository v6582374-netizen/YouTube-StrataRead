...
| Update discovery | Poll every imported public channel's Atom feed on launch and then every **6 hours while the app is running**. Coalesce manual refreshes into the active run; allow no more than one manual run per **15 minutes**. A closed desktop app makes no promise of background delivery. | The official push-notification guide names this Atom URL as a channel topic, but push delivery needs a publicly reachable webhook, which a local-only desktop app cannot receive directly. Direct polling has no published rate contract, so serial, coalesced scans are safer than bursty fan-out. Six hours bounds ordinary staleness without turning a local app into a poller. |
...
- [Google: push notifications](https://developers.google.com/youtube/v3/guides/push_notifications) names `https://www.youtube.com/feeds/videos.xml?channel_id=CHANNEL_ID` as the Atom topic and requires a publicly reachable callback. It validates the feed identity but does not provide a direct-polling rate contract. A server-side relay is out of scope for this local-first release.
...
The Atom feed endpoint has no published direct-polling rate contract in these authoritative sources. The six-hour interval, serial scan, request spacing, and backfill cap are therefore conservative product defaults, not vendor-guaranteed limits. `yt-dlp` is not the YouTube Data API: its use and distribution must be reviewed against the applicable YouTube terms before release, and the product must not frame it as an official API path. These defaults never justify proxy rotation, access-control bypass, or other limit evasion.
